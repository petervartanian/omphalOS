...
SELECT
  ...
  importer_country AS importer_country,
  ship_month AS ship_month,
  SUM(value_usd) AS total_value_usd,
  COUNT(*) AS row_count
FROM j
WHERE status = 'REVIEW'
GROUP BY exporter_country, importer_country, ship_month
ORDER BY total_value_usd DESC, row_count DESC
LIMIT :limit;
